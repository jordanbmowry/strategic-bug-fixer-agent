...
export const parseTestOutput = (output) => {
	...
	if (stackMatch && stackMatch[1]) {
		...
		const isValid =
			!matchedFile.includes(".test.") && !matchedFile.includes("node_modules");

		if (isValid) {
			sourceFile = matchedFile.startsWith("./")
				? matchedFile
				: "./" + matchedFile;
		}
	}
...
export const createCIConfig = (options = {}) => {
	return Object.freeze({
		...
		commitMessage:
			options.commitMessage || "🤖 Auto-fix: Resolve test failures",
